...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        ...
        # self.cbam_up = OutConv(64, n_classes)

    def forward(self, x):
        # _, attn_s1 = self.cbam_h(x)
        x1 = self.inc(x)
        x2 = self.down1(x1)
        x3 = self.down2(x2)
        x4 = self.down3(x3)
        ...
        logits = self.outc(x)
        # _, attn_s2 = self.cbam_t(logits)
        # attn_score = self.cbam_up(cbam_64)
        return logits, x
...
class NestedUNet(nn.Module):
    def __init__(self, n_classes, n_channels=1, deep_supervision=False, **kwargs):
        super().__init__()
